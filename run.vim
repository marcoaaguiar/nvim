...
augroup python_run
    ...
    autocmd FileType python nnoremap <silent> <F5> :w <CR> :call MonkeyTerminalExec('poetry run python ' . expand('%'))<cr>
augroup END
...
augroup tex
    autocmd!
    " autocmd BufRead,BufNewFile *.go set filetype=go
    autocmd FileType go nnoremap <F5> :call <plug>(vimtex-compile)
augroup END
