...
augroup python_run
    autocmd!
    let g:monkey_terminal_startup = ['export PYTHONPATH=' . expand('$PWD') , '']
    " autocmd BufRead,BufNewFile *.py set filetype=python
    autocmd FileType python nnoremap <buffer> <silent> <F5> :w <CR> :call MonkeyTerminalExec('poetry run python ' . expand('%'))<cr>
    autocmd FileType python nnoremap <buffer> <silent> <F6> :w <CR> :call MonkeyTerminalExec('poetry run python -i ' . expand('%'))<cr>
    autocmd FileType python nnoremap <buffer> <silent> <F7> :call MonkeyTerminalExec('<C-d>')<cr>
augroup END
